...
func main() {
	...
	ws, err := websocket.Dial("ws://127.0.0.1:42424/echo", "ws://127.0.0.1")
	...
	/**
	ready(func() {
		canvas := dom.GetWindow().Document().QuerySelector("canvas").(dom.HTMLElement)
		canvas.Underlying().Call("webkitRequestFullscreen")
		canvas.SetAttribute("width", strconv.FormatInt(int64(dom.GetWindow().InnerWidth()), 10))
		canvas.SetAttribute("height", strconv.FormatInt(int64(dom.GetWindow().InnerHeight()), 10))
...
func ready(f func()) {
	...
	switch readyState := doc.ReadyState(); readyState {
	case "loading":
		doc.AddEventListener("DOMContentLoaded", false, func(e dom.Event) {
			go f()
		})
